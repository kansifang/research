...
(function () {
    ...
    a0.dom.contains = function (T, cB) {
        ...
        T = cC(T);
        cB = cC(cB);
        return T.contains ? T != cB && T.contains(cB) : !! (T.compareDocumentPosition(cB) & 16)
    };
    ...
    a0.string.format = function (cC, T) {
        ...
        if (cB.length) {
            cB = cB.length == 1 ? (T !== null && (/\[object Array\]|\[object Object\]/.test(cD.call(T))) ? T : cB) : cB;
    ...
    a0.dom.getStyle = function (cC, cB) {
        ...
        var cE = cC.style[cB];
        if (!cE) {
            var T = cF._styleFixer[cB],
                cD = cC.currentStyle || (a0.browser.ie ? cC.style : getComputedStyle(cC, null));
    ...
    a0.browser.isGecko = /gecko/i.test(navigator.userAgent) && !/like gecko/i.test(navigator.userAgent);
    a0.browser.isStrict = document.compatMode == "CSS1Compat";
    a0.dom.getPosition = function (T) {
        T = a0.dom.g(T);
        var cJ = a0.dom.getDocument(T),
            cD = a0.browser,
            cG = a0.dom.getStyle,
            cC = cD.isGecko > 0 && cJ.getBoxObjectFor && cG(T, "position") == "absolute" && (T.style.top === "" || T.style.left === ""),
            cH = {
                left: 0,
                top: 0
            },
            cF = (cD.ie && !cD.isStrict) ? cJ.body : cJ.documentElement,
            cK, cB;
        if (T == cF) {
            return cH
        }
        if (T.getBoundingClientRect) {
            ...
            cH.left = Math.floor(cB.left) + Math.max(cJ.documentElement.scrollLeft, cJ.body.scrollLeft);
            cH.top = Math.floor(cB.top) + Math.max(cJ.documentElement.scrollTop, cJ.body.scrollTop);
            cH.left -= cJ.documentElement.clientLeft;
            cH.top -= cJ.documentElement.clientTop;
            var cI = cJ.body,
                cL = parseInt(cG(cI, "borderLeftWidth")),
                cE = parseInt(cG(cI, "borderTopWidth"));
            ...
            if (cD.opera > 0 || (cD.isWebkit > 0 && cG(T, "position") == "absolute")) {
    ...
    a0.dom.hasClass = function (cC, cB) {
        ...
        var T = -1;
        try {
            T = cC.className == cB || cC.className.search(new RegExp("(\\s|^)" + cB + "(\\s|$)"))
        } catch (cD) {
            return false
        }
        return T > -1
    ...
    a0.extend(br.prototype, {
        ...
        _bind: function () {
            ...
            T._watchSize = function () {
                var cB = T.getSize();
                if (T.width != cB.width || T.height != cB.height) {
                    var cD = new aA(T.width, T.height);
                    var cE = new a8("onbeforeresize");
                    cE.size = cD;
                    T.dispatchEvent(cE);
                    T._updateCenterPoint((cB.width - T.width) / 2, (cB.height - T.height) / 2);
                    T.maskLayer.style.width = (T.width = cB.width) + "px";
                    T.maskLayer.style.height = (T.height = cB.height) + "px";
        ...
        _updateCenterPoint: function (cD, cB, cH, cG) {
            var cE = this.getMapType().getZoomUnits(this.getZoom());
            ...
            var cC = (cH && cG) ? cI.lngLatToMercator(cH, this.currentCity) : this.mercatorCenter;
            if (cC) {
                this.mercatorCenter = new b3(cC.lng + cD * cE, cC.lat - cB * cE);
                var T = cI.mercatorToLngLat(this.mercatorCenter, this.currentCity);
                if (T && cF) {
                    this.centerPoint = T
                }
            }
        ...
        removeControl: function (T) {
            if (T && F(T.remove)) {
                T.remove();
                this.dispatchEvent(new a8("onremovecontrol", T))
            }
        },
        addContextMenu: function (T) {
            if (T && F(T.initialize)) {
                T.initialize(this);
                this.dispatchEvent(new a8("onaddcontextmenu", T))
            }
        },
        removeContextMenu: function (T) {
            if (T && F(T.remove)) {
                this.dispatchEvent(new a8("onremovecontextmenu", T));
                T.remove()
            }
        },
        ...
        removeOverlay: function (T) {
            if (T && F(T.remove)) {
                T.remove();
                this.dispatchEvent(new a8("onremoveoverlay", T))
            }
        },
        ...
        removeTileLayer: function (T) {
            if (T) {
                this.dispatchEvent(new a8("onremovetilelayer", T))
            }
        },
        setMapType: function (cB) {
            ...
            cC.mapType = cB;
            this.dispatchEvent(cC);
            if (cB === BMAP_SATELLITE_MAP || cB === BMAP_HYBRID_MAP) {
                _addStat(5003)
            }
        },
            ...
            if (T instanceof b3) {
                ...
                if (bU(T)) {
                    ...
                    cB.setSearchCompleteCallback(function (cD) {
                        if (cB.getStatus() == 0 && cB._json.result.type == 2) {
        ...
        getSize: function () {
            return new aA(this.container.clientWidth, this.container.clientHeight)
        },
        getCenter: function () {
        ...
        pointToPixel: function (T, cB) {
            cB = cB || this.getZoom();
            return this.projection.pointToPixel(T, cB, this.mercatorCenter, this.getSize(), this.currentCity)
        },
        pixelToPoint: function (T, cB) {
            cB = cB || this.getZoom();
            return this.projection.pixelToPoint(T, cB, this.mercatorCenter, this.getSize(), this.currentCity)
        },
        ...
        getBounds: function () {
            ...
            var cB = arguments[0] || {},
                cD = cB.margins || [0, 0, 0, 0],
                T = cB.zoom || null,
                cE = this.pixelToPoint({
                    x: cD[3],
                    y: this.height - cD[2]
                }, T),
                cC = this.pixelToPoint({
                    x: this.width - cD[1],
                    y: cD[0]
                }, T);
            return new bE(cE, cC)
        ...
        _getBestLevel: function (cB, cC) {
            ...
            for (var cD = cK; cD >= T; cD--) {
                ...
                if (cB.toSpan().lng / cJ < this.width - cI && cB.toSpan().lat / cJ < this.height - cG) {
                    break
                }
            }
            cD += cE;
        ...
        getViewport: function (cJ, cB) {
            ...
            if (!cJ || !cJ instanceof bE && cJ.length == 0 || cJ instanceof bE && cJ.isEmpty()) {
                return cN
            }
            var cL = [];
            if (cJ instanceof bE) {
            ...
            for (var cG = 0, cE = cL.length; cG < cE; cG++) {
                cF.push(this.projection.lngLatToMercator(cL[cG], this.currentCity))
            }
            var cC = new bE();
            ...
            if (cB.margins) {
                var cI = cB.margins,
                    cH = (cI[1] - cI[3]) / 2,
                    cK = (cI[0] - cI[2]) / 2,
                    cD = this.getMapType().getZoomUnits(cM);
                T.lng = T.lng + cD * cH;
                T.lat = T.lat + cD * cK
            }
        ...
        setViewport: function (cB, cE) {
            ...
            cE = cE || {};
            var cC = cE.delay || 200;
            if (T.zoom == this.zoomLevel && cE.enableAnimation != false) {
                var cD = this;
                setTimeout(function () {
                    cD.panTo(T.center, {
                        duration: 210
    ...
    g.prototype._launch = function () {
        ...
        cC.schedule = cC._opts.transition((T - cC._beginTime) / cC._opts.duration);
        if (F(cC._opts.render)) {
    ...
    if (a0.browser.firefox) {
        ...
        if (a0.browser.chrome || a0.browser.safari) {
            a0.extend(b2, {
                ...
                defaultCursor: "url(" + b2.imgPath + "openhand.cur) 8 8,default",
                draggingCursor: "url(" + b2.imgPath + "closedhand.cur) 8 8,move"
            })
        } else {
            a0.extend(b2, {
                distCursor: "url(" + b2.imgPath + "ruler.cur),crosshair",
    ...
    var cn = {
        request: function (cB) {
            ...
            if (T.addEventListener) {
                ...
                if (T.attachEvent) {
                    T.attachEvent("onreadystatechange", function (cD) {
                        var cC = window.event.srcElement;
                        if (cC && (cC.readyState == "loaded" || cC.readyState == "complete")) {
                            cC.parentNode.removeChild(cC)
                        }
                    })
                }
                ...
                if (T._status == this.Request.INITIAL) {
                    ...
                    if (cC.delayFlag == false) {
                        ...
                        window.setTimeout(function () {
                            var cE = cC.Config._baseUrl + "&mod=" + cC.Module._arrMdls.join(",");
                            cn.request(cE);
    ...
    a0.extend(cf.prototype, {
        ...
        _i: function (T) {
            if (!this._container && this.initialize && F(this.initialize)) {
        ...
        _render: function () {
            this._container = am(this._map.container, "<div unselectable='on'></div>");
        ...
        setAnchor: function (cD) {
            if (this.anchorFixed || !aD(cD) || isNaN(cD) || cD < BMAP_ANCHOR_TOP_LEFT || cD > BMAP_ANCHOR_BOTTOM_RIGHT) {
            ...
            var cB = ["TL", "TR", "BL", "BR"];
            a0.dom.removeClass(this._container, "anchor" + cB[cC]);
    ...
    a0.extend(I.prototype, {
        ...
        setType: function (T) {
            if (aD(T) && T >= BMAP_NAVIGATION_CONTROL_LARGE && T <= BMAP_NAVIGATION_CONTROL_ZOOM) {
                this._opts.type = T
            } else {
    ...
    a0.object.extend(cp.prototype, {
        ...
        addItem: function (cC) {
            if (!cC || cC._type != "menuitem" || cC._text == "" || cC._width <= 0) {
                return
            }
        ...
        removeSeparator: function (cB) {
            ...
            for (var cC = 0, T = this._items.length; cC < T; cC++) {
                if (this._items[cC] && this._items[cC]._type == "divider" && this._items[cC]._dIndex == cB) {
    ...
    a0.object.extend(bE.prototype, {
        ...
        getCenter: function () {
            ...
            return new b3((this._swLng + this._neLng) / 2, (this._swLat + this._neLat) / 2)
        },
        intersects: function (cC) {
            if (!(cC instanceof bE)) {
                return null
            }
            if (Math.max(cC._swLng, cC._neLng) < Math.min(this._swLng, this._neLng) || Math.min(cC._swLng, cC._neLng) > Math.max(this._swLng, this._neLng) || Math.max(cC._swLat, cC._neLat) < Math.min(this._swLat, this._neLat) || Math.min(cC._swLat, cC._neLat) > Math.max(this._swLat, this._neLat)) {
                return null
            }
        ...
        toSpan: function () {
            ...
            return new b3(Math.abs(this._neLng - this._swLng), Math.abs(this._neLat - this._swLat))
        }
    });
    ...
    a0.extend(bW, {
        ...
        getLnglatIndex: function (cD, cH, cG) {
            ...
            for (var cE = 0; cE < this.correct_pts[cD].length; cE++) {
                var T = this.getDis(this.correct_pts[cD][cE].x, this.correct_pts[cD][cE].y, cH, cG);
                if (T < cF) {
                    if (T < cI) {
                        cF = cI;
                        cI = T;
        ...
        getOMapIndex_mm: function (cD, cI, cH) {
            ...
            var cG = 1294723000,
                cF = 1294723000;
            for (var cE = 0; cE < this.correct_pts[cD].length; cE++) {
                var T = this.getDis(this.correct_pts[cD][cE].utm_x, this.correct_pts[cD][cE].utm_y, cI, cH);
                if (T < cF) {
                    if (T < cG) {
                        cF = cG;
                        cG = T;
        ...
        toMap: function (cD, T, cE) {
            ...
            var cC = (T + cE) * this.num[cD].num * this.num[cD].num2;
            return {
                x: cB,
                y: cC
            }
        ...
        getDgPix_mm: function (cE, cJ, cF) {
            var cI = this.fromMap(cE, this.correct_pts[cE][cJ].x, this.correct_pts[cE][cJ].y);
            var cG = this.fromMap(cE, this.correct_pts[cE][cF].x, this.correct_pts[cE][cF].y);
            var cO = cI.x,
                cB = cI.y;
            var cN = cG.x,
                T = cG.y;
            var cL = this.correct_pts[cE][cJ].utm_x,
                cD = this.correct_pts[cE][cJ].utm_y;
            var cH = this.correct_pts[cE][cF].utm_x,
                cC = this.correct_pts[cE][cF].utm_y;
        ...
        getOMap_pts: function (cB, T) {
            return this.getOMap_index(cB, T.lng, T.lat, T.lt, T.rb)
        },
        getMapJw_pts: function (cB, T) {
            return this.getMapJw_index(cB, T.lng, 9998336 - T.lat, T.lt, T.rb)
        },
    ...
    a0.extend(a2, {
        ...
        getDistance: function (cB, T, cD, cC) {
            return this.EARTHRADIUS * Math.acos((Math.sin(cD) * Math.sin(cC) + Math.cos(cD) * Math.cos(cC) * Math.cos(T - cB)))
        },
    ...
    BMap.register(function (cB) {
        ...
        if (a0.browser.ie && a0.browser.ie < 8 || document.compatMode == "BackCompat") {
        ...
        cB.addEventListener("addoverlay", function (cG) {
            var cD = cG.target;
            if (cD instanceof S) {
        ...
        cB.addEventListener("removeoverlay", function (cF) {
            var cD = cF.target;
            if (cD instanceof S) {
        ...
        cB.addEventListener("clearoverlays", function (cF) {
    ...
    a0.extend(k.prototype, {
        _addDom: function () {
            ...
            if (T.borderColor != "none") {
                cB.push("border:" + T.lineStroke + "px " + T.lineStyle + " " + T.lineColor + ";")
            }
            cB.push("opacity:" + T.opacity + "; filter:(opacity=" + T.opacity * 100 + ")");
            cB.push("background:" + T.background + ";");
            cB.push('z-index:60;">');
            cB.push(cC);
            cB.push("</div>");
            this.domElement = am(this.map.getPanes().markerMouseTarget, cB.join(""))
        },
        initialize: function (T) {
            this.map = T;
            this._addDom();
            if (this.domElement) {
                a0.on(this.domElement, "mousedown", function (cB) {
                    aI(cB)
                })
            }
            return this.domElement
        ...
        draw: function () {
            ...
            this._config.offset = new aA(-Math.round(this._config.width / 2) - Math.round(this._config.lineStroke), -Math.round(this._config.height / 2) - Math.round(this._config.lineStroke));
    ...
    a0.extend(cd.prototype, {
        ...
        setPositionAt: function (cB, T) {
            ...
            this.points[cB] = new b3(T.lng, T.lat);
            if (cB == 0 && !this.points[0].equals(this.points[this.points.length - 1])) {
    ...
    a0.extend(a.prototype, {
        ...
        _getPerimeterPoints: function (T, cI) {
            ...
            for (var cG = 0; cG < 360; cG += 9) {
                var cE = (Math.PI / 180) * cG,
                    cL = Math.asin(Math.sin(cH) * Math.cos(cK) + Math.cos(cH) * Math.sin(cK) * Math.cos(cE)),
                    cJ = Math.atan2(Math.sin(cE) * Math.sin(cK) * Math.cos(cH), Math.cos(cK) - Math.sin(cH) * Math.sin(cL)),
                    cM = ((cN - cJ + Math.PI) % (2 * Math.PI)) - Math.PI,
                    cO = new b3(cM * (180 / Math.PI), cL * (180 / Math.PI));
                cP.push(cO)
            }
            var cC = cP[0];
    ...
    a0.extend(bI.prototype, {
        ...
        getTileName: function (T, cB) {
            if (this.map.getMapType() === BMAP_PERSPECTIVE_MAP) {
                return "TILE-" + cB.guid + "-" + this.map.cityCode + "-" + T[0] + "-" + T[1] + "-" + T[2]
            } else {
        ...
        this.img.onload = function (cO) {
            ...
            if (cF.img && !v(cF.img)) {
                if (cD.tilesDiv) {
                    ...
                    if (a0.browser.ie <= 6 && a0.browser.ie > 0 && cF._transparentPng) {
                        cF.img.style.cssText += ';filter: progid:DXImageTransform.Microsoft.AlphaImageLoader(src="' + cF.src + '",sizingMethod=scale);'
                    }
                }
            }
    ...
    aw.prototype.getTilesUrl = function (cG, cB) {
        var cH = "";
        if (this._opts.predictDate) {
            cH = "HistoryService?day=" + (this._opts.predictDate.weekday - 1) + "&hour=" + this._opts.predictDate.hour + "&t=" + new Date().getTime() + "&"
        } else {
        ...
        var cC = this._map,
            cI = cG.x,
            cD = cG.y,
            cF = Math.floor(cI / 200),
            cE = Math.floor(cD / 200),
            T = this._tileUrl + cH + "level=" + cB + "&x=" + cI + "&y=" + cD;
    ...
    var bX = ["http://q1.baidu.com/it/", "http://q2.baidu.com/it/", "http://q3.baidu.com/it/", "http://q4.baidu.com/it/", "http://q5.baidu.com/it/", "http://q6.baidu.com/it/", "http://q7.baidu.com/it/", "http://q8.baidu.com/it/"];
    var aM = new m();
    aM.getTilesUrl = function (cC, cF) {
        ...
        var cB = bX[Math.abs(cG + cD) % bX.length] + "u=x=" + cG + ";y=" + cD + ";z=" + cF + ";v=" + T + ";type=web&fm=" + cE;
    ...
    var bk = new m();
    bk.tileUrls = ["http://d0.map.baidu.com/resource/mappic/", "http://d1.map.baidu.com/resource/mappic/", "http://d2.map.baidu.com/resource/mappic/", "http://d3.map.baidu.com/resource/mappic/"];
    bk.getTilesUrl = function (T, cC) {
        var cE = T.x;
        var cB = T.y;
        var cD = Math.pow(2, (20 - cC)) * 256;
        cB = Math.round((9998336 - cD * (cB)) / cD) - 1;
        url = this.tileUrls[Math.abs(cE + cB) % this.tileUrls.length] + this.map.currentCity + "/" + this.map.cityCode + "/3/lv" + (21 - cC) + "/" + cE + "," + cB + ".jpg";
        return url
    };
    ...
    bH.getTilesUrl = function (cB, cD) {
        var cE = cB.x;
        var cC = cB.y;
        var T = bX[Math.abs(cE + cC) % bX.length] + "u=x=" + cE + ";y=" + cC + ";z=" + cD + ";v=009;type=sate&fm=46";
        return T.replace(/-(\d+)/gi, "M$1")
    };
    ...
    l.getTilesUrl = function (cB, cD) {
        var cE = cB.x;
        var cC = cB.y;
        var T = bX[Math.abs(cE + cC) % bX.length] + "u=x=" + cE + ";y=" + cC + ";z=" + cD + ";v=013;type=trans&fm=47";
        return T.replace(/-(\d+)/gi, "M$1")
    };
    ...
    var a3 = {
        ...
        request: function (cB, cH, T, cG) {
            ...
            var cF = this,
                cD = cF.REQ_BASE_URL + cG + "?" + cE + "&ie=utf-8&oue=1&res=api&callback=BMap._rd._cbk" + cC;
            cn.request(cD)
        }
    };
    BMap._rd = {};
    ...
    O.parseGeoExtReg1 = function (T) {
        var a = T.replace(/([1-9]\d*\.\d*|0\.\d*[1-9]\d*|0?\.0+|0|[1-9]\d*),([1-9]\d*\.\d*|0\.\d*[1-9]\d*|0?\.0+|0|[1-9]\d*)(,)/g, "$1,$2;")
		//console.log(a);
		return a;
    };
    O.parseGeoExtReg2 = function (cB, T) {
        var cC = new RegExp("(((-?\\d+)(\\.\\d+)?),((-?\\d+)(\\.\\d+)?);)(((-?\\d+)(\\.\\d+)?),((-?\\d+)(\\.\\d+)?);){" + T + "}", "ig");
		//console.log(cC);
    ...
    var aW = function (cC, cB) {
        ...
        if (typeof cB.renderOptions.selectFirstResult != "undefined" && !cB.renderOptions.selectFirstResult) {
    ...
    a0.extend(aW.prototype, {
        ...
        setPageCapacity: function (T) {
            if (typeof T == "number" && !isNaN(T)) {
                this._opts.pageCapacity = T < 1 ? aW.DEFAULT_PAGE_CAPACITY : (T > aW.MAX_PAGE_CAPACITY ? aW.DEFAULT_PAGE_CAPACITY : T)
            } else {
    ...
    var bc = function (T, cD) {
        ...
        if (cB.highlightMode != BMAP_HIGHLIGHT_STEP && cB.highlightMode != BMAP_HIGHLIGHT_ROUTE) {
    ...
    bc.ROAD_TYPE = ["", "\u73af\u5c9b", "\u65e0\u5c5e\u6027\u9053\u8def", "\u4e3b\u8def", "\u9ad8\u901f\u8fde\u63a5\u8def", "\u4ea4\u53c9\u70b9\u5185\u8def\u6bb5", "\u8fde\u63a5\u9053\u8def", "\u505c\u8f66\u573a\u5185\u90e8\u9053\u8def", "\u670d\u52a1\u533a\u5185\u90e8\u9053\u8def", "\u6865", "\u6b65\u884c\u8857", "\u8f85\u8def", "\u531d\u9053", "\u5168\u5c01\u95ed\u9053\u8def", "\u672a\u5b9a\u4e49\u4ea4\u901a\u533a\u57df", "POI\u8fde\u63a5\u8def", "\u96a7\u9053", "\u6b65\u884c\u9053", "\u516c\u4ea4\u4e13\u7528\u9053", "\u63d0\u524d\u53f3\u8f6c\u9053"];
    ...
    a0.extend(aQ.prototype, {
        getPoint: function (/*address*/T, /*callback*/cC, /*city*/cB) {
            this._queryList.push({
                method: "getPoint",
                arguments: [T, cC, cB]
            })
        },
        getLocation: function (/*address*/T, /*callback*/cC, /*city*/cB) {
            this._queryList.push({
                method: "getLocation",
                arguments: [T, cC, cB]
            })
